                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            alert('לינק זום עודכן בהצלחה!')
                          } catch (error) {
                            alert('שגיאה בעדכון לינק זום')
                          }
                        }}
...
function AdminPage() {
  ...
  const loadData = async (retryCount = 0) => {
    ...
    try {
      ...
      alert(`שגיאה בטעינת הנתונים: ${errorMessage}`)
  ...
  const handleDeleteCategory = async (id) => {
    if (!confirm('האם אתה בטוח שברצונך למחוק קטגוריה זו? כל המסלולים בקטגוריה זו יימחקו גם כן.')) {
      return
    }
    try {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!file.type.startsWith('video/')) {
      alert('אנא העלה קובץ וידאו בלבד (MP4, MOV, AVI וכו\')')
      e.target.value = ''
      return
    }
    ...
    try {
      ...
      alert(`שגיאה בהעלאת הקובץ: ${error.message}. אנא נסה שוב.`)
    }
  }
  ...
  const addVideoToCategory = () => {
    if (!newCategoryVideo.title || !newCategoryVideo.url) {
      alert('אנא מלא כותרת וקישור לסרטון')
      return
    }
    setCategoryForm({
      ...categoryForm,
  ...
  const handlePurchaseSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 
                          ...
                          error.message || 
                          'שגיאה ביצירת הרכישה'
      alert(`שגיאה ביצירת הרכישה: ${errorMessage}`)
    }
  }
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 
                          ...
                          error.message || 
                          'שגיאה ביצירת הפגישה'
      alert(`שגיאה ביצירת הפגישה: ${errorMessage}`)
    }
  }
