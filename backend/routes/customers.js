...
router.get('/admin/customers', async (req, res, next) => {
  try {
    ...
    const customersWithStats = customers.map(customer => {
      ...
      const completedPurchases = customer.purchases.filter(p => p.status === 'completed').length
      
      return {
        ...customer,
        stats: {
          totalSessions: customer.bookings.length,
          confirmedSessions: confirmedBookings,
          completedCourses: completedPurchases,
          totalSpent: customer.purchases.reduce((sum, p) => sum + (p.price || 0), 0)
        }
      }
    })
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        initialPassword: initialPassword // מחזיר את הסיסמה הראשונית למנהל
      }
    })
  } catch (error) {
